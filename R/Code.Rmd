...
This .Rmd document provides the code I used for data processing, data anlysing, and visualization.

# Data model

A relational data model organizes extracted data from included studies into tables of rows and columns, and the relationships between the data are defined using primary and foreign keys
includes my local data tables. This allows for efficient querying and data manipulation, and makes it easy to integrate data from multiple tables.
...
Now I want to create a new column called "es_ID" which matches all the pairs of measurement I will use in the escalc() function to calculate the effect size measurements.
Study, PFAS type, species, and sampling location must be the same, so I group by them. 
The group (i.e., group_info) must be different, so that I pair adult-progeny in all possible combinations (different tissues in adults, eggs, chicks).
...
Remove effect sizes where both adult and progeny concentrations were under the limit of detection or quantification. 
This was an assumption made in the pre-registration. 
This is necessary to have meaningful response ratios.
...
This is necessary for successful analysis because means cannot be "NA" in effect size calculations.
...
Here I create three data sets to run a sensitivity analysis.
The first data set (i.e., dat) will include all the data.
The second (i.e., *dat2*) one will drop out data that are from pooled samples (i.e., from adult male + female pooled data) and/or data from adults and progeny that come from different nests. 
The third one (i.e., *dat3*) will drop out all effect size measures where the PFAS concentration in the adult OR progeny is below the limit of detection/quantification.
Eval is false in the following chunks.
...
This caterpillar plot does not tell us anything new that the orchard plot didn't. But it can be compared to the orchard plot.
```{r,eval=FALSE}
...
The effects of the other grouping variable are assumed to be random and are accounted for in the analysis. This allows the model to account for any differences between the groups and to provide more accurate estimates of the relationships between the variables.

## Meta-regression model

Now that the effects of the grouping variables are modeled as random effects, we include in the model additional variables (moderators) to explain any variations in the results. Moderators are tested for their overall effect on the maternal PFAS transfer.
The moderators I am going to include in the model are:

- type of progeny's tissue
- type of adult's tissue
- bird species (*this is probably wrong because it is a random effect already*)
...
### PFAS type carboxylic acids type

**Here I have a problem**
If I use VCV_lnRR and then I filter the data, R gives me the error 
"rror in rma.mv(lnRR, VCV_lnRR, mod = ~pfas_type_acronym, random = list(~1 |  : 
  Length of 'yi' (239) and length/dimensions of 'V' (371) is not the same."
Because obviously yi and V have different length/dimensions.
PLUS,
I dont know if all this make sense. lol.
For now, I put everything in eval = FALSE.
...
summary(model6)
i2_ml(model6)
orchard_plot(model6, mod = "pfas_type_acronym", group = "study_ID", data = dplyr::filter(dat, functional_group == "Carboxylate / carboxylic acid"), 
             xlab = "lnRR")
```

### PFAS sulfonic acids type
...
summary(model7)
i2_ml(model7)
orchard_plot(model7, mod = "pfas_type_acronym", group = "study_ID", data = dat, xlab = "lnRR")
```
...
model8 <- rma.mv(lnRR,
              ...
              test = "t",
              data = dat)
save(model7, file = here("Rdata", "model8.RData"))
```

```{r}
load(here("RData", "model8.RData"))
summary(model8)
i2_ml(model8)
orchard_plot(model8, mod = "seabird", group = "study_ID", data = dat, xlab = "lnRR")
```
