...
chick <-tdat[tdat$group_info == "progeny", 1]

ids <- expand.grid(adult_id = adult, chick_id = chick)
...
pos_chick <- match(check_id, tdat$measurement_ID)
chick_dat <- tdat[pos_chick, ]

#ndat<-as.data.frame(cbind(adult_dat, chick_dat))

ndat <- data.frame(measurement_ID_A = adult_dat[["measurement_ID"]],
           measurement_ID_C = chick_dat[[ "measurement_ID"]],
           mean_arithmetic_A = adult_dat[["mean_arithmetic"]],
           mean_arithmetic_C =  chick_dat[[ "mean_arithmetic"]]
             )
...
chick <-dat[dat$group_info == "progeny", 1]
...
pos_chick <- match(check_id, dat$measurement_ID)
chick_dat <- dat[pos_chick, ]
...
test.list <- list(tdat, tdat, tdat, tdat)
