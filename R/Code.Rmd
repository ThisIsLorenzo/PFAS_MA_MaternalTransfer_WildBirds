...
lnRR (point estimate) and var_lnRR ([sampling] variance estimate) forumulas are from Hedges et al. (1999) and subsequently extended by Lajeunesse (2011) and Senior et al. (2020).
...
# Making the same in a data set without albumen.
...
plot_sample_sizes <- ggplot(table_sample_sizes_df, aes(x=variable,
  ...
  scale_y_continuous(name = "n (sample size)",
  ...
  theme(legend.position = "none",
       ...
       panel.grid.minor = element_line(color = "grey", 
                                      ...
                                      linetype = "dotted"))
plot_sample_sizes
# ggsave(here("RData", "plot_samplesizes.png"),
#        width = 8,
#        height = 6)
...
Make same plot but semplified - albumen and others
...
subset(dat, dat$sample_type_P == "albumen")$study_ID
#albumen comes from only 1 study: S_002
```

#### Sensitivity analysis
Excluding albumen from the model.
```{r, eval=FALSE}
albumen_model_sensitivity <- run_model(dat_noalbumen, ~ sample_type_P)

save(albumen_model_sensitivity, file = here("Rdata", "albumen_model_sensitivity.RData"))
```

```{r}
load(here("RData", "albumen_model_sensitivity.RData"))
summary(albumen_model_sensitivity)
#r2_ml(albumen_model_sensitivity, dat_noalbumen, boot = 10)
#                 Est.  2.5% 97.5%                                                                
# R2_marginal    0.036 0.011 0.085
# R2_conditional 0.516 0.342 0.684

orchard_plot(albumen_model_sensitivity,
             mod = "sample_type_P",
             group = "study_ID", 
             data = dat_noalbumen,
             xlab = "lnRR")
...
Even removing albumen,  the results of the Test of Moderators still suggest that there is a relationship between the moderators and the dependent variable (p-val = 0.0201). However, the results for the predictor variables suggest that there is no significant relationship between the predictor variables and the dependent variable.
...
subset(dat, dat$diet == "Insectivorous")$study_ID
...
```{r, eval=FALSE}
diet_model_sensitivity <- run_model(dat_noparus, ~ diet)

save(diet_model_sensitivity, file = here("Rdata", "diet_model_sensitivity.RData"))
```

```{r}
load(here("RData", "diet_model_sensitivity.RData"))
summary(diet_model_sensitivity)
#r2_ml(diet_model_sensitivity, dat_noparus, boot = 10)
#                 Est.  2.5% 97.5%                                                                
# R2_marginal    0.047 0.040 0.065
# R2_conditional 0.071 0.043 0.138

orchard_plot(diet_model_sensitivity,
             mod = "diet",
             group = "study_ID", 
             data = dat_noinsectiv,
             xlab = "lnRR")
...
```{r, eval=FALSE}
body_weight_model_sensitivity <- run_model(dat_noparus, ~ body_weight_mean)

save(body_weight_model_sensitivity, file = here("Rdata", "body_weight_model_sensitivity.RData"))
```

```{r}
load(here("RData", "body_weight_model_sensitivity.RData"))
summary(body_weight_model_sensitivity)
#r2_ml(body_weight_model_sensitivity, dat_noparus, boot = 10)
#                 Est.  2.5% 97.5%                                                                  
# R2_marginal    0.002 0.000 0.009
# R2_conditional 0.038 0.001 0.091
```
...
```{r, eval=FALSE}
clutch_size_model_sensitivity <- run_model(dat_noparus, ~ clutch_size)

save(clutch_size_model_sensitivity, file = here("Rdata", "clutch_size_model_sensitivity.RData"))
```

```{r}
load(here("RData", "clutch_size_model_sensitivity.RData"))
summary(clutch_size_model_sensitivity)
#r2_ml(clutch_size_model_sensitivity, dat_noparus, boot = 10)
#                 Est. 2.5% 97.5%                                                                   
# R2_marginal    0.001    0 0.007
# R2_conditional 0.007    0 0.042
```
...
```{r}
sum(is.na(dat_noparus$egg_weight_mean))
#[1] 28
dat_egg_weight2 <- dat_noparus[complete.cases(dat_noparus[ , "egg_weight_mean"]),] #removing NAs
sum(is.na(dat_egg_weight2$egg_weight_mean))
#[1] 0
```

```{r, eval = FALSE}
egg_weight_model_sensitivity <- run_model(dat_egg_weight2, ~ egg_weight_mean)

save(egg_weight_model_sensitivity, file = here("Rdata", "egg_weight_model_sensitivity.RData"))
```

```{r}
load(here("RData", "egg_weight_model_sensitivity.RData"))
summary(egg_weight_model_sensitivity)
#r2_ml(egg_weight_model_sensitivity, dat_egg_weight2, boot = 10)
#                 Est.  2.5% 97.5%                                                        
# R2_marginal    0.001 0.000 0.015
# R2_conditional 0.033 0.002 0.072

bubble_plot(egg_weight_model_sensitivity,
             mod = "egg_weight_mean",
             group = "study_ID",
             data = dat_egg_weight2, 
             xlab = "egg_weight_mean")
```
...
dat2 is data_pooled == "No" & nest == "Yes"

```{r, eval=FALSE}
full_model0_sensitivity2 <- rma.mv(yi = lnRR,
                      V = VCV_lnRR_dat2, 
                      mods = ~1 + 
                        functional_group +
                        scale(carbon_chain_length) +
...
save(full_model0_sensitivity2, file = here("Rdata", "full_model0_sensitivity2.RData"))
```

```{r}
load(here("RData", "full_model0_sensitivity2.RData"))
summary(full_model0_sensitivity2)
```
...
Plot
```{r}
weights <- read_csv(here("Data", "sum_of_weights_AIC.csv")) #upload data from sw(mod.candidate)
...
orchaRd::bubble_plot(pb2_bubble, 
                     data = dat,
                     group = "study_ID",
                     mod = "ess.se",
                     xlab = "ess.se", 
