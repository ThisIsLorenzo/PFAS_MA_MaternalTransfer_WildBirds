...
I need to estimate these missing SDs. 
To do so, I follow the methods proposed by Nakagawa et al., 2022 ([A robust and readily implementable method for the meta-analysis of response ratios with and without missing standard deviations](https://ecoevorxiv.org/repository/view/3753/)).
...
To implement the all-cases Method, we need to first calculate the between study CV. We will do this with the `cvg_avg` function. 
Here, we need to do this for the adults and progeny groups as follows, such that variable b_CV_1 and b_CV_2 are the averages for the adults and progeny groups, respectively:
...
save(model1_noalbu, file = here("Rdata", "model1_noalbu.RData"))
```


```{r}
load(here("RData", "model1_noalbu.RData"))
summary(model1_noalbu)
i2_ml(model1_noalbu)
orchard_plot(model1_noalbu, group = "study_ID", data = dplyr::filter(dat, sample_type_P != "albumen" ), xlab = "lnRR")
```


```{r, eval=FALSE}
I2_noalbu <- orchaRd::i2_ml(model1_noalbu)

orchaRd::orchard_plot(model1_noalbu,
                      mod="1",
                      group = "study_ID",
                      data = dplyr::filter(dat, sample_type_P != "albumen" ),
                      xlab = "lnRR") + 
  annotate(geom="text",
           x= 0.80,
           y= 1, 
           label= paste0("italic(I)^{2} == ", round(I2_noalbu[1],4)), # I want to use % but cannot do should come back
              color="black", parse = TRUE, size = 5) +
  scale_fill_manual(values="grey") +
  scale_colour_manual(values="grey")
```
...
Now
Determine the randome effect structure filtering out albumen
```{r, eval=FALSE}
model2_noalbu <- rma.mv(lnRR, var_lnRR, # Add `VCV_lnRR` to account for correlated errors between cohorts (shared_controls)
              random = list(~1|study_ID, # Identity of the study
                            ~1|Phylogeny, # Phylogenetic correlation between species
                            ~1|species_ID, # Identity of the species
                            ~1|pfas_ID, # Type of PFAS
...
save(model2_noalbu, file = here("Rdata", "model2_noalbu.RData"))
```


```{r}
load(here("RData", "model2_noalbu.RData"))
summary(model2_noalbu)
i2_ml(model2_noalbu)
orchard_plot(model2_noalbu, group = "study_ID", data = dplyr::filter(dat, sample_type_P != "albumen" ), xlab = "lnRR")
```
...
dat$carbon_chain_length[dat$carbon_chain_length == "mixed"] <- NA

dat$carbon_chain_length <- as.numeric(dat$carbon_chain_length)

model_ccl <- rma.mv(lnRR, var_lnRR,
...
summary(model_ccl)
i2_ml(model_ccl)
bubble_plot(model_ccl, mod = "carbon_chain_length", group = "study_ID", data =dat , xlab = "lnRR")
```

### PFAS molecular weight
```{r, eval=FALSE}
dat$molecular_weight[dat$molecular_weight == "mixed"] <- NA
dat$molecular_weight <- as.numeric(dat$molecular_weight)

model_mw <- rma.mv(lnRR, VCV_lnRR,
...
summary(model_mw)
i2_ml(model_mw)
bubble_plot(model_mw, mod = "molecular_weight", group = "study_ID", data =dat, xlab = "lnRR")


cor.test(dat$carbon_chain_length, dat$molecular_weight)
```


### PFAS type carboxylic acids type

**Here I have a problem** If I use VCV_lnRR and then I filter the data, R gives me the error

`"rror in rma.mv(lnRR, VCV_lnRR, mod = ~pfas_type_acronym, random = list(~1 | : Length of 'yi' (239) and length/dimensions of 'V' (371) is not the same."`
