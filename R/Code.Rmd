...
where mT and mC are the sample means for the treatment (progeny) and control (adults) groups, respectively, nT and nC are
the sample sizes for the two groups and CV is the coefficient of variation; that is, CVC = sdT /mT and CVC = sdC/mC, with sd being the (sample) standard deviation.
...
Spake and Doncaster (2017) showed that sampling variance esti-
mates that include sample standard deviation are
likely to be downwardly biased when sample sizes (n) are small (n < 50).
Our sample sizes (n) per effect size were between 1 and 38, with the
average of 24.3. *these values are PROBABLY WRONG* - Should I use N_tilde (min, max, mean)?
...
ggplot(aes(x = moderator, 
     ...
     geom_point(aes(size=(1/sqrt(var_lnRR)), 
...
ccl_model <- run_model(dat, ~carbon_chain_length)
...
```{r, eval=FALSE}
mw_model <- run_model(dat, ~molecular_weight)
...
```{r, eval = FALSE}
mw_model_no_alb <- run_model(dat_no_alb, ~ molecular_weight)

save(mw_model_no_alb, file = here("Rdata", "mw_model_no_alb.RData"))
```

```{r}
load(here("RData", "mw_model_no_alb.RData"))
summary(mw_model_no_alb)
i2_ml(mw_model_no_alb)
bubble_plot(mw_model_no_alb,
            mod = "molecular_weight", 
            group = "study_ID", 
            data = dat_no_alb,
            xlab = "molecular_weight")
...
```{r, eval=FALSE}
mmr_model2 <- run_model(dat, ~ functional_group - 1 + scale(carbon_chain_length))
...
- Run a sensitivity analysis without pooled data (male+female) and adulta and progeny from different nests (i.e., dat2)

Meta-analytic model

```{r, eval=FALSE}
ma_model1_dat2 <- rma.mv(yi = lnRR,
             V = VCV_lnRR_dat2 , 
             random = list(~1|study_ID, 
                            ~1|Phylogeny, 
                            ~1|species_ID, 
...
save(ma_model1_dat2, file = here("Rdata", "ma_model1_dat2.RData"))
```

```{r, eval=FALSE}
load(here("RData", "ma_model1_dat2.RData"))
summary(ma_model1_dat2)
i2_ml(ma_model1_dat2)
orchard_plot(ma_model1_dat2,
             group = "study_ID", 
             data = dat2, 
             xlab = "lnRR")
```
