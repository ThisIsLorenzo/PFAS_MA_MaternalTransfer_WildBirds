...
```{r visualization, results = 'hide', include = FALSE}
data_dm_all_keys %>% 
  dm_draw()
```
...
##Table of results and orchard plot.
...
orchaRd::orchard_plot(model1_res,
                      mod="1",
                      xlab = "lnRR") + 
  annotate(geom="text",
           x= 0.80,
...
From previous graphs we can see that overall estimate from a random-effects meta-analysis of 371 effect sizes is centered on zero, with a 95% CI that spans the line of no-effect.
The prediction intervals clearly demonstrate the high level of heterogeneity (over 95%).
...
```{r, eval=FALSE}
model2 <- rma.mv(lnRR, VCV, # Add `var_lnRR` to account for correlated errors between cohorts (shared_controls)
              random = list(~1|study_ID, # Identity of the study
                            ~1|Phylogeny, # Phylogenetic correlation between species
                            ~1|species_ID,
                            ~1|pfas_ID, # Type of PFAS
...
```{r, eval=FALSE}
model3 <- rma.mv(lnRR, VCV, # Add `var_lnRR` to account for correlated errors between cohorts (shared_controls)
...
```{r, eval=FALSE}
model4 <- rma.mv(lnRR, VCV, # Add `var_lnRR` to account for correlated errors between cohorts (shared_controls)
              mod = ~ sample_type_A,
              random = list(~1|study_ID, # Identity of the study
                            #~1|Phylogeny, # Phylogenetic correlation between species
                            ~1|species_ID,
