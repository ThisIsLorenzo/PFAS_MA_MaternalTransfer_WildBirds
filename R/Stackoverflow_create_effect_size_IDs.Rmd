...
df <- 
  data.frame(var1 = rep(LETTERS[1:3], #study_ID
                        each = 4), 
             var2 = rep(LETTERS[4:6], #pfas_ID
                        each = 2), 
             var3 = c('F', 'F', 'F', 'F', 'G', 'G'), #species_ID
             var4 = c('H', 'H', 'I', 'I', 'L', 'L'), #sampling_location
             var5 = c('M', 'N', 'O', 'N', 'M', 'P', 'M', 'N', 'O', 'N', 'M', 'N'), #tissue_type
             var6 = c('x', 'y', 'x', 'y', 'x', 'y'), #group_ID
             var7 = c('', '', '', '', '', '')) #es_ID
df
